...
\caption{Extract Method Refactoring in Java}}
...
\begin{enumerate}
    ...
    \caption{Demonstration of ownership implications in Rust Code}
    }
    ...
    However, these references can be immutable (\icodeverb{&T}) or mutable
    (\icodeverb{&mut T}), and only one active mutable reference is allowed at a
    ...
    \begin{subfigure}[t]{0.47\textwidth}
        ...
        \captionsetup{justification=centering}
        \sublistingcaption{An Immutable Borrow \\ A program can have any number of references to it}
    ...
    \begin{subfigure}[t]{0.47\textwidth}
        ...
        \captionsetup{justification=centering}
        \sublistingcaption{A Mutable Borrow \\ A program can only have one reference to it}
        \label{lst:borrow-mutable}
    \end{subfigure}
    \captionsetup{type=listing,justification=centering}
    \caption{Example of Borrowing in Rust. Somewhat similar to sharing values via pointers in C, but safe!}
    ...
    When the programmer writes \icodeverb{echo(message: \&str) -> \&str}, the
    ...
    So, behind the scenes, Rust sees that \icodeverb{message} is a reference that
    lives at least as long as \icodeverb{'a}, and it infers that the output must
    share that same \icodeverb{'a} lifetime. The programmer doesn't need to write
    \icodeverb{<'a>} or annotate the references explicitly, because the language
    ...
    references, forcing programmers to specify such \icodeverb{'a} parameters
    explicitly.

    \item \textbf{Non-local Control Flow}
    Rust allows \icodeverb{return}, \icodeverb{break}, and \icodeverb{continue} inside
    code blocks. Extracting a fragment of code containing, for example, an early
    \icodeverb{return}, into a function changes the flow of control unless handled
    explicitly.
...
For example, one pass systematically encodes \icodeverb{return}, \icodeverb{break},
and \icodeverb{continue} statements into an auxiliary enum, allowing the caller
function to reconstruct the original control flow by matching on variants such
as \icodeverb{Ok(...)}, \icodeverb{Return(...)}, etc. Another pass utilizes
...
support for asynchronous programming constructs (\icodeverb{async}/\icodeverb{await})
...
\texttt{CHARON} and \texttt{AENEAS}\cite{aeneas, AENEAS_PART_2} can assist by
...
\subsection{Build a Proof-of-Concept Extension for VSCode}
