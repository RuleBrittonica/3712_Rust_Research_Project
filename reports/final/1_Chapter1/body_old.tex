...
system — centred on ownership and lifetimes — poses
...
model checking), and efforts to formalise Rust's semantics. We compare Rust's
...
pinpointing structural refinements for a clearer organisation of this body of
work.

\section{Overview of Refactoring}

As previously mentioned, refactoring is the process of restructuring existing
code to improve its internal structure without changing its external behaviour.
It is a key practice for enhancing code maintainability and reducing technical
...
refactorings, providing developers with quick, semantics- persevering code
...
intra-procedural \textbf{ownership analysis} which infers whether each value
moved to the new function should be passed by value, by shared reference (using
\icodeverb{&T}), or by mutable reference (using \icodeverb{&mut T}). Appendix 1
...
control flow (e.g. \icodeverb{return} or \icodeverb{break} inside the extracted fragment) and borrowing
...
\cite{RustLancet}. Under the hood, it uses Rust parsing libraries (\icodeverb{syn} and
\icodeverb{quote}) to manipulate the AST and generate patched code. In a similar
manner to REM, Rust-Lancet employs an interative repair loop: it first applies a
fix (for example, inserting a \icodeverb{clone()} on a value to appease ownership
rules), then rechecks the compiler errors and repeats if fruther fixes are
...
prover of choice. For example, one might translate Rust to Coq or F* and
...
for interactive use \cite{Prusti_assistant_github}.
...
internal compiler data structures that change nightly \cite{RustLancet}.
